...
Назначение:
    ...
    По умолчанию работает с sold_ads (отличия от reparse_cian_offerdata.py,
    который для active_ads). Флаг --active переключает обратно на active.

Что делает:
    1. SELECT external_id FROM sold_ads
       WHERE source='cian_active' AND raw_data::jsonb->'offer' IS NULL
       (т.е. ещё не перепарсено)
       Опционально --since 'YYYY-MM-DD' фильтрует по sold_date.
    2. Для каждого: POST /v2/cian/scrape → parse через CianSource → upsert
       sold_ads.raw_data = полный offerData
...
Идемпотентность:
    Условие фильтра: raw_data::jsonb->'offer' IS NULL.
...
Параметры:
    ...
    --concurrency N   : параллельных запросов к flippercrawl (default 8)
    --ids a,b,c       : только эти external_id (ручной smoke, force reparse)
    --active          : парсить active_ads вместо sold_ads
    --since YYYY-MM-DD: только sold_ads с sold_date >= этой даты

Использование:
    # Smoke на 10 снятых
    py -3.11 -m scripts.reparse_cian_sold_offerdata --limit 10

    # Все снятые, последние 90 дней
    py -3.11 -m scripts.reparse_cian_sold_offerdata --since 2026-05-01

    # Все снятые (18 375) — concurrency 12
    py -3.11 -m scripts.reparse_cian_sold_offerdata --concurrency 12

    # Конкретные ids
    py -3.11 -m scripts.reparse_cian_sold_offerdata --ids 328760023,324782461
"""
...
async def _fetch_ad_ids(
    ...
    """SELECT external_id из sold_ads (по умолчанию) или active_ads которые ещё не перепарсены.

    only_sold=True → sold_ads WHERE source='cian_active' (то, что деактивировалось через pipeline).
        Опционально фильтр по дате since='YYYY-MM-DD' по sold_date.
    only_sold=False → active_ads (как в оригинальном reparse_cian_offerdata).
    """
    dsn = _to_asyncpg_dsn(local_url)
    conn = await asyncpg.connect(dsn)
    try:
        if only_sold:
            # Продано. Фильтр «ещё не перепарсено» — по ключу offer в raw_data.
            where = ["source='cian_active'", "raw_data::jsonb->'offer' IS NULL"]
            params = []
            if since:
                where.append("sold_date >= $%d" % (len(params) + 1))
                params.append(since)
            where_sql = " AND ".join(where)
            sql = f"""
                SELECT external_id FROM sold_ads
                WHERE {where_sql}
                ORDER BY sold_date DESC NULLS LAST, id
            """
            if limit:
...
async def _update_raw_data(local_url: str, external_id: str, raw_data: dict, sold: bool = True) -> int:
    """UPDATE raw_data для одного ad. Без нормализации полей — только raw_data."""
    dsn = _to_asyncpg_dsn(local_url)
    conn = await asyncpg.connect(dsn)
    try:
        new_raw = json.dumps(raw_data, ensure_ascii=False)
        table = "sold_ads" if sold else "active_ads"
...
async def _process_one(
    ...
    # Update raw_data
    try:
        n = await _update_raw_data(local_url, external_id, raw_offer, sold=sold)
        if n > 0:
            stats["updated"] += 1
        else:
            stats["update_noop"] += 1
...
async def run(
    ...
    target = "sold_ads" if only_sold else "active_ads"
    logger.info("== reparse_cian_offerdata (SOLD) START ==" if only_sold else "== reparse_cian_offerdata START ==")
    ...
    ad_ids = await _fetch_ad_ids(local_url, limit, only_ids, only_sold=only_sold, since=since)
    ...
    async def _worker(ext_id: str) -> None:
        async with sem:
            if _SHUTDOWN.is_set():
                return
            await _process_one(source, local_url, ext_id, stats, sold=only_sold)
...
def main() -> int:
    ...
    parser.add_argument("--active", action="store_true",
                        help="Парсить active_ads вместо sold_ads (поведение как в "
                             "оригинальном reparse_cian_offerdata.py)")
    parser.add_argument("--since", default=None,
                        help="Только sold_ads с sold_date >= YYYY-MM-DD (default: без фильтра)")
