...
class CianParser:
    def __init__(self, location: str, proxies=None, list_html_loader=None):
        """
        ...
        :param proxies: список URL прокси для прямых GET страниц списка (curl_cffi), default None
        :param list_html_loader: optional callable(url: str) -> str — внешняя загрузка HTML списков
        """

        location_id = __validation_init__(location)

        self.__parser__ = None
        self.__list_html_loader__ = list_html_loader
    ...
    def __set_proxy__(self, url_list):
        ...
        if available_proxy is not None:
            self.__session__.proxies = {
                "http": available_proxy,
                "https": available_proxy,
            }

    def __load_list_page__(
        self, url_list_format, page_number, attempt_number_exception
    ):
        url_list = url_list_format.format(page_number)
        if self.__list_html_loader__ is None:
            self.__set_proxy__(url_list)

        if page_number == self.__parser__.start_page and attempt_number_exception == 0:
            print(
                f"The page from which the collection of information begins: \n {url_list}"
            )

        if self.__list_html_loader__ is not None:
            return self.__list_html_loader__(url_list)

        res = self.__session__.get(url=url_list, timeout=60)
    ...
    def __run__(self, url_list_format: str):
        ...
        while page_number < self.__parser__.end_page and not end_all_parsing:
            ...
            while attempt_number_exception < 3 and not page_parsed:
                try:
                    (page_parsed, attempt_number, end_all_parsing) = (
                        self.__parser__.parse_list_offers_page(
                            ...
                            page_number=page_number,
                            count_of_pages=self.__parser__.end_page
                            + 1
                            - self.__parser__.start_page,
                    ...
                    print(f"\n\nException: {e}")
                    print(
                        f"The collection of information from the pages with ending parse on {page_number} page...\n"
                    )
                    break
        ...
        print(
            f"Total number of parsed offers: {self.__parser__.count_parsed_offers}. ",
            end="\n",
        )
...
def __validation_init__(location):
    ...
    if location_id is None:
        ValueError(
            f"You entered {location}, which is not exists in base."
...
def __validation_get_flats__(deal_type, rooms):
    if deal_type not in DEAL_TYPES:
        raise ValueError(
            f"You entered deal_type={deal_type}, which is not valid value. "
    ...
    if type(rooms) is tuple:
        for count_of_room in rooms:
            if type(count_of_room) is int:
                if count_of_room < 1 or count_of_room > 5:
                    raise ValueError(
                        f"You entered {count_of_room} in {rooms}, which is not valid value. "
                ...
                raise ValueError(
                    'In tuple "rooms" not valid type of element. '
                    'It is correct int and str types. Example (1,3,5, "studio").'
                )
    elif type(rooms) is int:
        if rooms < 1 or rooms > 5:
            raise ValueError(
                f"You entered rooms={rooms}, which is not valid value. "
                f'Try entering one of these values: 1, 2, 3, 4, 5, "studio", "all".'
            )
        ...
        raise ValueError(
            'In argument "rooms" not valid type of element. '
            'It is correct int, str and tuple types. Example 1, (1,3, "studio"), "studio, "all".'
        )
...
def __validation_get_suburban__(suburban_type, deal_type):
    if suburban_type not in OBJECT_SUBURBAN_TYPES.keys():
        raise ValueError(
            f"You entered suburban_type={suburban_type}, which is not valid value. "
