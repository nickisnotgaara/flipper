// Column defs + filter defs for the "Активные" sheet of the tables
// page. Kept in a sibling _sheets/ folder (the underscore prefix tells
// Next.js not to treat it as a route) so all four sheets live in one
// place and can be switched in a single Tabs component on the page.
...
// Hard-coded count keeps the tab badge stable while the API is
// warming up. The DataTable's own stats will override the display
// value inside the table area.
export const ACTIVE_TOTAL = 5_227;

export const activeFilters: FilterDef[] = [
  {
    key: 'rooms',
    label: 'Комнат',
    group: 'Категории',
...
function daysTone(d: number | null | undefined): { color: string } {
  if (d == null) return { color: 'text-zinc-400' };
  if (d <= 7) return { color: 'text-emerald-700' };
  if (d <= 30) return { color: 'text-amber-700' };
  if (d <= 90) return { color: 'text-orange-700' };
  return { color: 'text-rose-800' };
}

export const activeColumns: ColumnDef<any, any>[] = [
  {
    id: 'title',
    header: 'Адрес / район',
    accessorKey: 'title',
  ...
  },
  {
    ...
    cell: ({ row }: any) => {
      ...
      return (
        <div className="text-right">
          <span
            className={`font-mono tabular-nums text-[12.5px] font-semibold ${tone.color}`}
          >
            {d ?? '—'}
          </span>
        </div>
